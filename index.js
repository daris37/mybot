...
/////////////////
const BotName = '☭METABOT☭'; 
const instagram = 'https://www.instagram.com/metabot.id/'; 
const telegram = '-'; 
const kapanbotaktif = 'KETIKA HANYA DIAKTIFKAN'; 
const youtube = 'https://www.youtube.com/channel/UCt3fVZppeLJpmpeXeuke2cQ';
...
conn.on('user-presence-update', json => console.log(json.id + ' presence is => ' + json.type)) || console.log('Bot by ig:@metabot.id')
conn.on('message-status-update', json =>
{
   const participant = json.participant ? ' (' + json.participant + ')' : '' // participant exists when the message is from a group
   console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by ig:@metabot.id`)
})
